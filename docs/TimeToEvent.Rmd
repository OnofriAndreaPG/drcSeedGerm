...
In our previous page about censoring we ideally took a seed population, with log-normal distribution of germination time, location parameter $\mu = 4.5$ days, scale parameter $\sigma = 0.6$  and a maximum germinated proportion $\pi = 0.85$. For this population, we time to 50% germination (T50) was 5.14 days and the Mean Germination Time (MGT) was 5.39 days. We took a sample of 100 seeds from that population, by using Monte Carlo simulation. For the ease of reading, we replicate the simulation here.



```{r}
#Real parameter values
mu <- 4.5; sigma <- 0.6; pMax = 0.85
