...
HTE3.fun <- function(time, Psi, G, Psib, sigmaPsib, thetaH, b){
...
"HTE3" <- function(){
fct <- function(x, parm){
  S <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                parm[,4], parm[,5])
  return(S)
}
...
ss <- function(data){
  ...
  modGR <- drm(GR50 ~ psiLevels, fct=GRPsiPol())
...
GR <- function(parms, respl, reference="control", type="relative", Psi){
  ...
  if(type=="absolute"){
    ...
    .GR50 <- ifelse(.GR50>0, .GR50, 0)
     ...
     EDp <- 1/res
    .GR <- EDp
     gra <- function(G, Psib, sigmaPsib, thetaH, b)  1/exp( - (1/b)*log( (1/g)* (G * (1 - exp( - (Psi - Psib) * (1/sigmaPsib)))  - g)) + log( thetaH/(-(Psi^2 - Psib^2))) )

    #Beginning of derivatives (finite differences) ###########################
    d1.1 <- gra(G, Psib, sigmaPsib, thetaH, b)
    d1.2 <- gra(G + 10e-6, Psib, sigmaPsib, thetaH, b)
    d1 <- (d1.2 - d1.1)/10e-6

    d1.1 <- gra(G, Psib, sigmaPsib, thetaH, b)
    d1.2 <- gra(G, Psib + 10e-6, sigmaPsib, thetaH, b)
    d2 <- (d1.2 - d1.1)/10e-6

    d1.1 <- gra(G, Psib, sigmaPsib, thetaH, b)
    d1.2 <- gra(G, Psib, sigmaPsib + 10e-6, thetaH, b)
    d3 <- (d1.2 - d1.1)/10e-6

    d1.1 <- gra(G, Psib, sigmaPsib, thetaH, b)
    d1.2 <- gra(G, Psib, sigmaPsib, thetaH + 10e-6, b)
    d4 <- (d1.2 - d1.1)/10e-6

    d1.1 <- gra(G, Psib, sigmaPsib, thetaH, b)
    d1.2 <- gra(G, Psib, sigmaPsib, thetaH, b + 10e-6)
    d5 <- (d1.2 - d1.1)/10e-6
    #End of derivatives #########################
...
deriv1 <- function(x, parm){
  ...
  d1.1 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                   parm[,4], parm[,5])
  d1.2 <- HTE3.fun(x[,1], x[,2], (parm[,1] + 10e-6), parm[,2], parm[,3],
                   parm[,4], parm[,5])
  d1 <- (d1.2 - d1.1)/10e-6

  d2.1 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                   parm[,4], parm[,5])
  d2.2 <- HTE3.fun(x[,1], x[,2], parm[,1], (parm[,2] + 10e-6), parm[,3],
                   parm[,4], parm[,5])
  d2 <- (d2.2 - d2.1)/10e-6

  d3.1 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                   parm[,4], parm[,5])
  d3.2 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], (parm[,3] + 10e-6),
                   parm[,4], parm[,5])
  d3 <- (d3.2 - d3.1)/10e-6

  d4.1 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                   parm[,4], parm[,5])
  d4.2 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                   (parm[,4] + 10e-6), parm[,5])
  d4 <- (d4.2 - d4.1)/10e-6

  d5.1 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                   parm[,4], parm[,5])
  d5.2 <- HTE3.fun(x[,1], x[,2], parm[,1], parm[,2], parm[,3],
                   parm[,4], (parm[,5] + 10e-6))
  d5 <- (d5.2 - d5.1)/10e-6
